...
function applyState(next: State): void {
  ...
  else if (next === 'RESUMING') setStatusInfo(`resuming — sending "${settings.resumePrompt}"`);
...
function handleEvent(ev: SessionWireEvent): void {
  switch (ev.type) {
    ...
    case 'resuming':
      log(`↻ Resuming · attempt ${ev.attempt} · sending "${settings.resumePrompt}"`);
      break;
    case 'resumed':
...
function openSettings(): void {
  ...
  el<HTMLInputElement>('setResumePrompt').value = settings.resumePrompt;
...
function saveSettings(): void {
  ...
  settings.resumePrompt = el<HTMLInputElement>('setResumePrompt').value.trim() || 'continue';
