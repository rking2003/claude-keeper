...
function wireSession(win: BrowserWindow): void {
  ...
  const persistWait = (): void => {
    try {
      waitStore().save({
        ...
        replayPrompt: controller?.replayPrompt,
      });
    } catch {
  ...
  const onResumeAfter = (_e: unknown, delayMs: number): void => {
    controller?.resumeAfter(delayMs);
  };
  const onSetAutoResume = (_e: unknown, enabled: boolean): void => controller?.setAutoResume(enabled);
  const onSetReplayPrompt = (_e: unknown, text: string): void => controller?.setReplayPrompt(text);
  ...
  const onReady = (): { recovered: boolean } => {
    ...
    c.recoverWaiting(
      pending.resetTimeMs === null ? null : new Date(pending.resetTimeMs),
      pending.replayPrompt,
    );
  ...
  ipcMain.on(IPC.sessionResumeAfter, onResumeAfter);
  ipcMain.on(IPC.sessionSetAutoResume, onSetAutoResume);
  ipcMain.on(IPC.sessionSetReplayPrompt, onSetReplayPrompt);
  ...
  win.on('closed', () => {
    ...
    ipcMain.removeListener(IPC.sessionResumeAfter, onResumeAfter);
    ipcMain.removeListener(IPC.sessionSetAutoResume, onSetAutoResume);
    ipcMain.removeListener(IPC.sessionSetReplayPrompt, onSetReplayPrompt);
  });
}

app.whenReady().then(() => {
  // Configure logging first so every subsequent milestone is captured. Basic
  // info/warn/error always go to the console + an append-only log file under the
  // app data dir; verbose (debug) detail is enabled via CLAUDE_KEEPER_VERBOSE or
  // the persisted `verboseLogging` setting.
