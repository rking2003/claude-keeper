...
export class ResumeScheduler {
  ...
  /**
   * Begin waiting a fixed delay from now — used by a user-set manual timer. Unlike
   * {@link start}, no safety buffer is added: the caller asked for an exact delay,
   * so we fire at `now + delayMs` (clamped to a non-negative delay).
   */
