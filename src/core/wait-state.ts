 ...
 */
import type { ResumeStrategy } from './session-controller';
...
export interface PendingWait {
  ...
  /** The prompt to replay (only meaningful for the 'replay' strategy). */
  replayPrompt: string;
}
...
export function buildPendingWait(input: PendingWaitInput): PendingWait {
  const strategy: ResumeStrategy = input.strategy === 'replay' ? 'replay' : 'continue';
  const prompt = (input.replayPrompt ?? '').slice(0, MAX_PROMPT_LEN);
  return {
...
export function parsePendingWait(raw: unknown): PendingWait | null {
  ...
  return buildPendingWait({
    ...
    strategy: o['strategy'] === 'replay' ? 'replay' : 'continue',
    ...
    replayPrompt: o['replayPrompt'] as string | undefined,
  });
}
