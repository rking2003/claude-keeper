...
describe('mergeSettings', () => {
  ...
  it('overlays provided fields and keeps defaults for the rest', () => {
    ...
    expect(out.resumePrompt).toBe(DEFAULT_SETTINGS.resumePrompt);
  ...
  it('normalizes the resume prompt: defaults, trims, and caps length', () => {
    expect(mergeSettings({}).resumePrompt).toBe('continue');
    expect(mergeSettings({ resumePrompt: '  keep going  ' }).resumePrompt).toBe('keep going');
    expect(mergeSettings({ resumePrompt: '' }).resumePrompt).toBe('continue');
    expect(mergeSettings({ resumePrompt: '   ' }).resumePrompt).toBe('continue');
    expect(mergeSettings({ resumePrompt: 7 }).resumePrompt).toBe('continue');
    expect(mergeSettings({ resumePrompt: 'x'.repeat(9999) }).resumePrompt.length).toBe(500);
  });
...
describe('SettingsStore', () => {
  ...
  it('round-trips: save then load yields the cleaned settings', () => {
    ...
    const written = store.save({ command: 'claude', resumePrompt: 'keep going', safetySec: 90, customPatterns: ['x'] });
    expect(written.resumePrompt).toBe('keep going');
  ...
  it('save normalizes/clamps invalid input before persisting', () => {
    const io = memIO();
    const written = new SettingsStore(io).save({ safetySec: -99, maxRetries: 9999, resumePrompt: 42 });
