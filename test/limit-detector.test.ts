...
describe('LimitDetector', () => {
  ...
  it('reassembles an ANSI escape split across the chunk boundary', () => {
    // PTY reads split at arbitrary byte offsets, so a styled banner can arrive with
    // its escape sequence cut in half. Per-chunk stripping used to leave residue
    // ("session <ESC>[" + "0mlimit") that fractured the phrase and missed the limit.
    const d = new LimitDetector({ now });
    // The split lands inside "\x1b[0m", between "session " and "limit".
    expect(d.push("\x1b[1mYou've hit your session \x1b[")).toBeNull();
    const ev = d.push('0mlimit · resets 9:00 AM (UTC).\r\n');
    expect(ev).not.toBeNull();
    expect(ev!.matchedText).toContain('session limit');
    expect(ev!.resetTime?.toISOString()).toBe('2026-01-15T09:00:00.000Z');
  });

  it('detects a styled "hit your session limit" banner delivered byte by byte', () => {
    // Worst case: every character in its own chunk, escapes fully fragmented.
    const banner =
      "\x1b[1mYou've hit your session \x1b[0mlimit \xb7 resets 9:00 AM (UTC)\x1b[0m\r\n";
    const d = new LimitDetector({ now });
    let ev = null;
    for (const ch of banner) ev = d.push(ch) ?? ev;
    expect(ev).not.toBeNull();
    expect(ev!.matchedText).toContain('session limit');
    expect(ev!.resetTime?.toISOString()).toBe('2026-01-15T09:00:00.000Z');
  });

  it('a withheld match is still seen via hasPendingMatch when an escape is split', () => {
    // The idle-flush safety net relies on hasPendingMatch(); a split escape must
    // not hide the pending candidate from it either.
    const d = new LimitDetector({ now });
    d.push("\x1b[1mYou've hit your session \x1b[");
    d.push('0mlimit · resets 9:00 AM (UTC)'); // no newline: withheld
    expect(d.hasPendingMatch()).toBe(true);
    const ev = d.flush();
    expect(ev!.resetTime?.toISOString()).toBe('2026-01-15T09:00:00.000Z');
  });
  ...
  describe('pre-limit percentage warnings (do NOT trigger the wait flow)', () => {
    it('ignores "used 71% of your limit" even though it pairs limit + reset', () => {
      const d = new LimitDetector({ now });
      expect(
        d.push("You've used 71% of your limit · resets at 9:00 AM (UTC).\n"),
      ).toBeNull();
    });

    it('ignores an approaching-limit warning with a decimal percentage', () => {
      const d = new LimitDetector({ now });
      expect(d.push('Approaching usage limit — 92.5% used, resets 5pm (UTC)\n')).toBeNull();
    });

    it('does not arm an idle flush for an unterminated percentage warning', () => {
      const d = new LimitDetector({ now });
      expect(d.push("You've used 71% of your limit · resets at 9:00 AM (UTC).")).toBeNull();
      expect(d.hasPendingMatch()).toBe(false);
      expect(d.flush()).toBeNull();
    });

    it('STILL triggers at 100% (limit actually reached)', () => {
      const d = new LimitDetector({ now });
      const ev = d.push("You've used 100% of your limit · resets at 9:00 AM (UTC).\n");
      expect(ev).not.toBeNull();
      expect(ev!.resetTime?.toISOString()).toBe('2026-01-15T09:00:00.000Z');
    });

    it('skips the warning line but still detects a real limit later in the stream', () => {
      const d = new LimitDetector({ now });
      expect(d.push("You've used 71% of your limit · resets at 9:00 AM (UTC).\n")).toBeNull();
      const ev = d.push('Claude usage limit reached. reset at 11:00 AM (UTC).\n');
      expect(ev).not.toBeNull();
      expect(ev!.resetTime?.toISOString()).toBe('2026-01-15T11:00:00.000Z');
    });

    it('emits the real limit when a warning and a reached-limit line arrive together', () => {
      const d = new LimitDetector({ now });
      const ev = d.push(
        "You've used 88% of your limit · resets at 9:00 AM (UTC).\n" +
