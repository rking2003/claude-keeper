...
describe('ResumeScheduler', () => {
  ...
  it('startIn() fires after an exact delay with no safety buffer added', () => {
    const s = make({ safetyBufferMs: 5000 }); // buffer must be ignored by startIn
    s.startIn(3000); // fireAt = now + 3000 exactly
  ...
  it('startIn() with a non-positive delay fires immediately', () => {
    const s = make();
    s.startIn(0);
    expect(resumes()).toHaveLength(1);
    s.stop();
