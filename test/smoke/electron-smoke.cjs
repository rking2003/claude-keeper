...
// `save` echoes the payload back in the shape main would return.
ipcMain.handle('settings:save', (_e, s) => ({ settings: s, patternErrors: [] }));
    ...
    const scaffold = JSON.parse(
      await read(
        'JSON.stringify({' +
          ...
          '["start","write","resize","stop","resumeNow","setAutoResume","ready","onEvent"].every(k => typeof window.keeper.session[k] === "function"),' +
    ...
    const saveRoundtrip = JSON.parse(
      await read(
        '(async () => JSON.stringify(await window.keeper.settings.save(' +
          '{ command: "claude", resumePrompt: "continue", customPatterns: ["x"] })))()',
      ),
    );
    if (saveRoundtrip.settings.resumePrompt !== 'continue') return fail('settings.save did not round-trip');
